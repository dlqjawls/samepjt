...
def test_admin_login_success(client):
    ...
    assert "accessToken" in json_response  # JWT 토큰이 반환되어야 함
    assert "refreshToken" in json_response  # 리프레시 토큰도 있어야 함


def test_admin_login_invalid_admin(client):
    """
    (Fail) 존재하지 않는 관리자 로그인 시 실패
    ...
    assert {"field": "adminId", "message": "Admin ID does not exist"} in json_response["detail"]["errors"]
...
def test_admin_login_wrong_password(client):
    ...
    assert response.status_code == 401
    assert json_response["detail"]["resultCode"] == "FAILURE"
    assert {"field": "adminPassword", "message": "Incorrect password"} in json_response["detail"]["errors"]
